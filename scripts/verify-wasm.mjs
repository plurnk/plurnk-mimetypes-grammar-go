#!/usr/bin/env node
// CI byte-identical reproducibility check for go.wasm.
...
const committed = await readFile(path.join(repoRoot, "go.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed go.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-go-verify-"));
...
console.log(`rebuilt go.wasm sha256: ${rebuiltHash}`);
