#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-go.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-go-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-go.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as go.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "go.wasm"));
const bytes = (await readFile(path.join(repoRoot, "go.wasm"))).length;
console.log(`go.wasm: ${bytes} bytes (built from ${pin})`);
